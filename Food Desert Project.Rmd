...
```
After running through all linear models the results showed us that the Region variable comes back as NA meaning this variables is useless to the model and does not seem to impact our dependent variable. We know that the difference between model_forward_adjr2 and model_forward_Cp is TractWhite, the summary of model_forward_adjr2 indicated that TractWhiteshould not be included in the model because it is not statistically significant, and anova() function's F test gave the same conclusion. When comparing model_forward_Cp to to model_forward_bic we can see that  model_forward_Cp is a better fit model based on the anova results. The model_forward_Cp fit uses all the variables except for TractBlack and TractWhite. We will keep this in mind and see how the backwards, sequential, and exhaustive methods align or contradict the results from forwards selection. 
