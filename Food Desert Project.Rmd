...
str(food)
#plot(food$TractBlack, food$Desert, xlab="Black Population",ylab="Probability of being located in Food Desert")
#lines(food$TractBlack,food$Desert)
#plot(Desert~TractBlack, data=food, col = "red")
#fit =glm(Desert~TractBlack, data=food, family="binomial") 
#predicted= predict(fit, data=food, type="response")
...
qqline(food$POP2010)
# We can tell from our Histogram and Q-Q plot that the distributon of the population is right-skewed or positive.
...
In summary,  only variable 'OHU2010' and 'TractWhite' in dataset food.desert showed p-values larger than 0.01, so we can say the two variables are normally distributed but we may not conclude that the other variables here are not normally distributed directly, we still need to combine plots to make a final conclusion.
...
# Deciding Best Variable with Forward Selection Not including Highly Correlated Variables 
reg.forward.2 <- regsubsets(Desert~.-CensusTract-State-OHU2010-TractWhite-TractKids-TractLOWI-TractOMultir, data=food, nvmax=19, nbest=1, method="forward")
plot(reg.forward.2, scale = "adjr2", main = "Adjusted R^2")
plot(reg.forward.2, scale = "Cp", main = "CP")
plot(reg.forward.2, scale = "bic", main = "BIC")
...
#Model A everything except CensusTract
model.a <- glm(Desert~.-CensusTract, data=food, family="binomial")
#summary(model.a)
#exp(model.a$coefficients)
# Everything except CensusTract and State
model.b <- glm(Desert~.-CensusTract-State, data=food, family="binomial")
# Using Exhaustive Feature Selection Model 
model.exhausitve <- glm(Desert~.-CensusTract-TractSNAP-TractBlack-TractHispanic, data=food, family="binomial")
...
# Model from bestglm() 
bestglm.1 <- glm(Desert~Urban+NUMGQTRS+PovertyRate+MedianFamilyIncome+TractLOWI+TractBlack+TractAsian+TractNHOPI+TractOMultir+TractHUNV, data=food, family= binomial)
# Model from bestglm() 2 
bestglm.2 <- glm(Desert~ Urban+NUMGQTRS+PovertyRate+MedianFamilyIncome+TractSeniors+TractBlack+TractAsian+TractAIAN+TractHispanic+TractHUNV+TractSNAP, data=food, family=binomial)
# Model from bestglm() 3 
bestglm.3 <- glm(Desert~ State+Urban+NUMGQTRS+PovertyRate+MedianFamilyIncome+TractLOWI+TractAsian+TractNHOPI+TractAIAN+TractOMultir+TractHUNV, data=food, family= binomial)
# Model from bestglm() 4 
bestglm.4 <- glm(Desert~ Urban+OHU2010+NUMGQTRS+PovertyRate+MedianFamilyIncome+TractLOWI+TractAsian+TractAIAN+TractOMultir+TractHispanic+TractHUNV, data=food, family=binomial)
# Model AIC Levels 
models <- list(model.a, model.b, model.exhausitve, model.corr, glm_newfood_fac_Desert, emptyglm,forwards_2, forwards2_corr, backwards_2, backwards2_corr, model.lit, bestglm.1, bestglm.2, bestglm.3, bestglm.4)
model.names <- c('model.a', 'model.b', 'model.exhaustive','model.corr','glm_newfood_fac_Desert', 'emptyglm','forwards_2', 'forwards2_corr', 'backwards_2', 'backwards2_corr','model.lit','bestglm.1', 'bestglm.2', 'bestglm.3','besglm.4')
...
#NNK Outlier Detection Using Kth Nearest Neighbour Distance Method
...
#Bind Data
library(dplyr)
#food_K <- bind_rows(Desertfactor, State, Urban, POP2010,NUMGQTRS, 
#    PovertyRate,MedianFamilyIncome,TractLOWI,TractKids,
#   TractSeniors,TractAsian,TractAIAN,TractHispanic, 
#    TractHUNV,TractSNAP)
summary(food_K)
