...
We chose to use the State variable because its representation is the most significant and it has obvious levels. The first step conducted was deciding to draw a dataset with a size of 7,280 rows to reflect the original dataset of 72,864 rows. Then we used the *table()* function to see the proportion of the State variable. Unfortunately, our results were not very satisfying. To simplify this process we used the calculations as seen below: ni is the number of observations for State level i; ceil stands for ceiling functions

$$a_i = ceil\left(\frac{n_i}{100}\right) x 10$$
$$N = \sum_{i=1}^{51}n_i = 71,864$$
$$A = \sum_{i=1}^{51}a_i = 7,280$$ 
         
We used the ceiling function because ai is not integers; we take ceiling values for ai to protect the integrality of levels with few rows. After this, “Sampling” package was installed loaded, strata() function was used to get stratified samples named as food. (See below for a readout of the dataset’s structure and variable names). However, once we extracted the sample food from the original dataset, we realized that we had 47 observations where all of our variables were zero. To mend this issue in our analysis, we subsetted the *food* data frame to only include variables greater than zero (0>). As expected, this further decreased the data frame to 7,232 variables. For further analysis, we chose Desert as our single dependent variable. 
...
Here we added runif function to avoid repeat of numbers in variables, repear will lead to an error of K-S test. And since we are using random number generator from uniform distribution, it will not affect our results. Based on the results of K-S tests, the variables ‘OHU2010’ in the dataset ‘food.desert’ showed p-value larger than 0.01, we can say that the variables are normally distributed, but our final conclusions about the rest variables must be made together with graph results. This matches the Q-Q plot of the occupied housing units in the earlier section that showed a normal distribution. 
...
We tried exhaustive method here as an additional part, this method is only available when number of possible predictor variables is less than 20. Since we have removed all the variables that we have seen from the other methods we definetly have less than 20 variables to find the best fit model. However, after we take these varibales out and look athe adjusted R-squared, Cp, and BIC results the best fit models show all the other variables. Therefore, we do not need to run a regression with these variables because they are the same for each criteria. 
...
The results above give us the top five models based on the criteria we selected. The next section will break down each model along with the models chosen from forwards selection to determine the best fit model from the AIC level. 

# Chapter 4.2: Model Selection 
## Step-Wise Selection with Dependent Variable as a Factor 
To continue with model selection we wanted to test whether or not the logistic regression makes a difference when we classify the Desert variable as a factor with two levels. Here we are running the regressions to further determine the best fit model through step-wise by adding variables onto an empty general model. The empty genereal model is the intercept-only model. We start with this model to do forward selection to better assess what variables to use for the best fit model. Additionally, we realize that regardless if the variable 'Desert' is numerical or categorical, we get the same results, but for stringency, we will use variable 'Desertfactor' in the dataset named as 'newfood'.
```{r}
newfood<-food
...
The results of the step-wise function for the forwards and backwards directions showed us different models. However, the forwards direction showed that the variable 'TractNHOPI' is not statistically singnificant with a p-value of 0.136183. Thus, we will use this model but take out this variable and this will match the resutls from the backwards direction. Furthermore, the results from classifying the direction as 'both' ways still ended up with the same results as the forwards direction. Moving to the next step of model building will we only be using the forwards model withouth TractNHOPI variable and we will call this forwards_2.
