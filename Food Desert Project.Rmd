...
To answer this question the first step was to subset our data to include two samples: one with food deserts and the other with no food deserts. This is similiar to the earlier EDA that we did, but we need a data frame that has no food deserts in order to execute the analysis. 
...
## Linear Model Try
### Here we are dissecting which variabels we should use in our model based on Adjusted R-Squared, Cp, BIC, and multicollinearity. 
...
# Chapter 3.2: Logistic Regression and Classification 
### Here we are performing a logistical regression and k-NN classification test to determine which variables impact whether or not a census tract is located in a food desert. 
```{r}
