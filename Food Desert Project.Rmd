...
Food insecurity continues to be a pressing issue throughout the United States. The terminology of ‘food deserts’ has consistently been used to describe neighborhoods with limited access to grocery stores or supermarkets. Studies have shown that the most vulnerable populations reside in areas with limited access to full-service grocery outlets. The literature surrounding food insecurity suggests that food deserts emerge from numerous sources including: urban sprawl, residents’ socioeconomic status, racial disparities, and neighborhood density. Research has shown that communities in food deserts are at a higher risk of health issues such as obesity and diabetes. While food deserts have become a legislative priority to address health inequities, there is no concrete evidence suggesting that increasing access to food retail would improve health issues in these communities. 

Food insecurity continues to be a pressing issue across the United States. The terminology of ‘food deserts’ has consistently been used to describe neighborhoods with limited access to grocery stores or supermarkets. Studies have shown that the most vulnerable populations reside in areas with limited access to full-service grocery outlets (Walker, 2012). The literature around food insecurity suggests that food deserts emerge from numerous sources including: urban sprawl, residents’ socioeconomic status, racial disparities, and neighborhood density (Dutko et al., 2012). Research shows that communities in food deserts are at higher risk of health issues including obesity and diabetes. While food deserts have become a legislative priority to address health inequities, there is no concrete evidence suggesting that increasing access to food retail would improve health issues in these communities (Block & Subramanian, 2015). 
...
17. TractNHOPI: Total number of Native Hawaiian or Other Pacific Islander in the tract 
...
Here we are looking at the normality of our quantitative independent variables to assessing whether or not we should exclude outliers when we build our model. We are assessing the normality of our variables from histograms and Q-Q plots. 

The following data analyses are separated by testing the normality of each one of the independent quantitative variables in our dataset. The literature regarding food deserts explains that low-income factors and racial minorities are related to food deserts because of urban sprawl and gentrification (Hamidi,2019). We are assessing the normality of these data points to ensure that when we run the respective analyses, our data is sufficient, and outliers have been normalized in scale to our data set. We are assessing the normality of our variables from histograms, Q-Q plots, and Kolmogorov-Smirnov test. 

### Poverty Rate Normality Test 
```{r Poverty Rate Normality, results= "hide"}
...
```{r Population Normality, results= "hide"}
...
When we look at the outliers of the *food* data, the mean with the outliers is 1,615 and without outliers the mean is 1,571. Clearly, not too much  changes by removing the outliers. Also, with and without outliers the distribution is skewed to the right. When looking at the outliers in our subset that only includes tracts in food deserts we can see that there are only 10 outliers. The mean with the outliers is 1,605 and without outliers is 1,579 so nothing really changes by removing the outliers.

### Group Housing Units Normality Test
```{r Group Housing Normality Test, results= "hide"}
...
When we subset our data to only census tracts that are located in food deserts the mean of the median family income decreased by more than \$20,000 showing about a 35% decrease in median family income. Furthermore, the maximum amount went from \$250,000 to \$140,250. Nonetheless, the distribution of the median family income for tracts that are located in food deserts is right-skewed or positive. Though, compared to the *food* data, the Q-Q plot does resembles more of a straight line, but is still right-skewed.
...
```{r Low-Income Population Normality Test, reults= "hide"}
...
When we subset our data to only census tracts that are located in food deserts the mean of the low-income population is 2,192, which is 719 more than the mean from the *food* data. This is worth pointing out because the subsetted data has fewer data points and a smaller maximum but the mean of low-income people in food deserts is higher. Nonetheless, the histogram and Q-Q plot of this variable shows us that the distribution is still right-skewed or positive.
...
When we look at the outliers in our *food* data we can see that the distribution is still right-skewed with and without outliers. Furthermore, the mean with outliers is about 1,472 and without outliers it is 1,353, so this did not change much. When we subset our *food* data to only include food deserts the mean with and without outliers changes from 2,192 to 2,075 so there is no significant change in removing outliers within this subset. Additionally, before and after the removal of outliers in the *food* data and subsetted data the distribution is still right-skewed or positive. 

### Kid Count Normality Test 
```{r Kid Count Normality Test, results= "hide"}
...
When we look at the outliers in our *food* data we can see that the distribution is still slightly skewed to the right with and without outliers. The mean with outliers is 1,025 and without outliers it is 960 so this did not change much. When we subset our *food* data to only include food deserts the mean with and without outliers changes from 1,097 to 1,026 which is certainly not a significant change. After we remove the outliers from both data frames we can see that our data is still right-skewed or positive; so essentially nothing changes. 

### Senior Count Normality Test 
```{r Senior Count Normality Test, results= "hide"}
...
When we subset our data to only census tracts that are located in food deserts the mean is 538 which is close to the mean of total senior count in the *food* data of 561. As such, one may assume that this variable is not too different based on whether or not a census tract is located in a food desert. However, the only way we can prove this is when run a regression is run later in the analysis. Nonetheless, the histogram and Q-Q plot of this variable show us that the distribution is still right-skewed or positive. 
...
```{r White Population Normality Test, results= "hide"}
...
***White Population Normality Test in only Food Deserts***
```{r, results= "hide"} 
...
```{r Black Population Normality Test, results= "hide"}
...
```{r Asian Population Normality Test, results= "hide"}
...
```{r Native Hawaiian and Pacific Islander Normality Test, results='hide'}
...
When we subset our data to only census tracts that are located in food deserts the mean is 8 which is higher than the mean of our *food* data but only by two people. However, what is interesting to note about the subsetted data is that the maximum amount for the *food* data and subset is the same at 1,333. This means that there are 1,333 people who identify as Native Hawaiian or Pacific Islander located in a food desert. Native and indigenous people are a minority population in the United States, yet the maximum amount of this population from our data does not have access to full service grocery stores. Unfortunately, we do not have enough information to determine if this variable is significant in our analysis, however we will discuss this later on in the report. Nonetheless, the Histogram and Q-Q plot of this variable show us that the distribution is still right-skewed or positive. 
...
```{r American Indian and Alaskan Native Population Normality Test, results='hide'}
...
When we subset our data to only census tracts that are located in food deserts the mean is 105, which is more than double the mean of the American Indian or Alaskan Native population in the *food* data. Furthermore, it is interesting to note that with this population like the Native Hawaiian or Pacific Islander variable, that the maximum total is the same for the *food* data and the subsetted data at 9,009. This means that a minority population, who is disproportionately represented with the rest of the country, and has a higher probability of being in a food desert--at least from our dataset. However, we do not have enough information to make this claim and will do further analysis on this later on in the report. Nonetheless, the Histogram and Q-Q plot of this variable show us that the distribution is still right-skewed or positive. 
...
When we look at the *food* data we can see that the distribution is still slightly skewed to the right with and without outliers. Furthermore, the mean with outliers is about 42 and without outliers it is about 18 so this did not change much. When we subset our *food* data to only include food deserts the mean with and without outliers changes from 105 to 22, which seems like a big change, but it is not significant enough to impact the distribution. The removal of outliers does not change the results of the data points enough to create a normal distribution. We can see that the distribution is still right-skewed or positive with the removal of outliers. 

### Multiple Races or Other Race Population Normality Test 
```{r Multiple race/other race population Normality Test, results='hide'}
...
When we subset our data to only census tracts that are located in food deserts the mean is about 439 which is only slightly larger than the *food* data mean of people who identify as multiple races or other. The nature of this variable is vague because it doesn't specify what races one identifies as, and the data source did not do a great job of explaining what is meant by the other aspect of this variable. As such, this variable might not be useful for the analysis. However, we do not have enough information to tell at this point and will decide later on in the report. Nonetheless, the Histogram and Q-Q plot of this variable show us that the distribution is still right-skewed or positive.   
...
When we look at the *food* data we can see that the distribution is still slightly skewed to the right with and without outliers. Furthermore, the mean with outliers is roughly 390 and without outliers it is roughly 245, so this did not change much. When we subset our *food* data to only include food deserts the mean with and without outliers changes from 439 to 322. Still, there is no significant change in removing outliers within the subset. The removal of outliers does not change the results of the data points enough to create a normal distribution. We can see that the distribution is still right-skewed or positive with the removal of outliers. 

### Hispanic Population Normality Test 
```{r Hispanic population Normality Test, results='hide'}
...
When we look at the *food* data we can see that the distribution is still slightly skewed to the right with and without outliers. Furthermore, the mean with outliers is about 707 and without outliers it is about 359 which appears to be a large drop. However, we can see above that the drop does not change the distribution. When we subset our *food* data to only include food deserts the mean with and without outliers changes from 862 to 420 which is more than 50% of a decrease, but still there is no significant change from the distribution. The removal of outliers does not change the results of the data points enough to create a normal distribution. We can see that the distribution is still right-skewed or positive with the removal of outliers. 

### Housing Units Without a Vehicle Normality Test 
```{r Housing units without access to a vehicle Normality Test, results='hide'}
summary(food$TractHUNV)
hist(food$TractHUNV, main="Histogram of Housing Units w/o a Vehicle", xlab="Total Housing Units w/o a Vehicle", col="orange")
qqnorm(food$TractHUNV, main="Q-Q Plot of Housing Units w/o a Vehicle")
...
When we look at the *food* data we can see that the distribution is still slightly skewed to the right both with and without outliers. Furthermore, the mean with outliers is about 150 and without outliers, it is 102 so this did not change much. When we subset our *food* data to only include food deserts the mean with and without outliers changes from 154 to 138, so there is no significant change in removing outliers within the subset. The removal of outliers does not change the results of the data points enough to create a normal distribution. We observe that the distribution is still right-skewed or positive with the removal of outliers. 

### Housing Units Receiving SNAP Normality Test 
```{r housing units receiving SNAP benefits Normality Test, reults='hide'}
...
Here, we added the runif function to avoid repetition of numbers in variables, as repeats will lead to an error of K-S test. And since we are using random number generator from uniform distribution, it will not affect our results. Based on the results of K-S tests, the variables ‘OHU2010’ in the dataset ‘food.desert’ showed p-value larger than 0.01, we can say that the variables are normally distributed, but our final conclusions about the rest variables must be made together with graph results. This matches the Q-Q plot of the occupied housing units in the earlier section that showed a normal distribution. 

# Chapter 2.1 Exploratory Data Analysis 
## Chi-Square Test of Independence 
Here, we are performing Chi-Square test on the categorical variables in our dataset to determine whether there is a relationship between our independent variables and dependent variable. 

```{r Categorical Variable Analysis_1, results='markup'}
...
Here, we mainly focus on the output of Chi-Square test - we got a test statistic of 53.899, which is larger than 3.84 (95% critical value of 1 df Chi-Square test), and the corresponding p-value is lower than 0.05. Thus, there is sufficient evidence to reject the null hypothesis. This means that the variables 'Urban' and 'Desert' are not independent of one another and in fact are related. Given this, we have good reason to keep variable 'Urban' in our model, but further investigations are needed. At this point, we have good reason to keep variable 'Urban' in our model, but further investigations are needed.

```{r Categorical Variable Analysis_2, results='markup'}
...
```{r Categorical Variable Analysis_3, results='markup'}
...
```
When running the chi-square test on the state variable and desert variable we can once again reject the null. The output here shows us that the test statistic is 297.53 which is much large than the critical chi-square value for 50 degrees of freedom at 67.505. The p-value is smaller than .05 so these results are statistically significant. 

The chi-square test showed us that we should be able to use the 'Region', 'Urban', and 'State' variables in our model as they will all have a relationship with our dependent variable. However, we will need to assess this further during the model building part of the report. 

# Chapter 2.2 
## Exploratory Data Analysis: Two Sample T-Test 
...
To answer this question the first step was to subset our data to include two samples: one with food deserts and the other with no food deserts. This is similar to the earlier EDA that we did, but we need a data frame that has excludes any food deserts in order to execute the analysis. 

```{R unpaired two-sample t test, results='hide'}
...
Overall, the results of our t-test align with the literature around food deserts, showing that racial minority populations are more likely to be in a census tract without access to a full-service grocery store along with low-income people who are at or below the federal poverty rate. We have multiple indicators of these populations and the results shed light on similarities with what previous research has described. The results of our t-test warrant further investigation into racial disparities, as well as the systemic and institutionalized racism that has plagued the United States thereby evidently leading to food insecurity along racial lines. 

# Chapter 2.3 Exploratory Data Analysis
## Correlation Matrix 
Here, we are plotting a correlation matrix to determine which of our variables are highly correlated with one another. We will use the matrix to help us determine what variables to use and control for multicollinearity.  The dependent variable in our data is a 0 or 1 indicator variable that determines whether or not a census tract is in a food desert, because we are running a logistical regression we knew that it is not necessary for us to turn the variable into a factor. Thus, we use it here in the correlation matrix, but realize that this does not tell us much based on the nature of the variable.  
...
From the correlation matrix we will look at variables that are at .7 or above to assess their correlation with one another. From this criteria, we can see that the population variables is highly correlated with the occupied housing units, the white population, and the total amount of kids in a census tract. Furthermore, the occupied housing variable is also highly correlated with the white population and total amount of kids in a census tract. The low-income population variable is highly correlated with the amount of people receiving SNAP benefits in a census tract. The multiple race/other variable is highly correlated with Hispanic$-$this is interesting because it seems that this variable would be correlated with all the other races, but it is only strongly correlated with the Hispanic tract variable. As such, we will be sure to keep this in mind as we are building our models for the logistic regression. 

# Chapter 3: Geographic Location and Dispersion of Data 
## Heatmaps 
In generating heatmaps, it is essential that both packages ‘usmap’ and ‘ggplot2’ are loaded within the library for further use. To aid in the process of creating heatmaps, a new dataset was created entitled ‘dataforheatmap’.
```{R Heatmap for population sum for different states}
dataforheatmap<-read.csv("dataforheatmap.csv")
...
The heatmap employs several variables from the set including ‘‘state’, ‘sumpop’ and ‘Desertp’. The ‘sumpop’ variable describes  the cumulative population(POP2010) of different states from our dataset. The ‘Desert’ variable is the proportion of food deserts (number of food deserts in the strata i of state as well as number of observations for each strata i of state), so this variable ranges between 0 and 1. A heatmap was generated for variable ‘POP2010’ in order to do a rough initial check on the accuracy of our data. For the ‘POP2010’ heatmap, it was observed that for states located in coastal areas/regions, these are more likely to have larger populations. Those representing inland parts, it was observed that a lesser population was assigned. Normally, for the Northeast area, we should observe a similar color to that of California and Texas States; however, we did not. Thus, a comparison between this graph and a population heat map from official resources(https://www.mapsofworld.com/usa/thematic-maps/usa-population-map.html) was conducted. 

It was observed rather obviously that some states have large populations in the USA Population Map (CA, TX, FL, GA, NC, NY, PA, OH, IL) in relation to other states. Our heatmap for variable ‘POP2010’ showed very similar results, yet the only difference is around NY state. The reasonable explanation in this instance is that there is a sampling bias in the sample of NY State, so it can be ignored for now, and discuss potential solutions for this issue later. Overall, our heatmap for population is good, which indicated that we were using the right dataset. We can furthermore deduce from this that our heatmap for Food Desert Proportion is also good. 

The results depicted by the graphs are easy to interpret upon first gaze. However, the data does suggest that some confounding factors are aroused and to be accounted for during future research if necessary. First, since our original dataset (sample size is ~ 72,000) was retrieved from census, which can be considered as sampling. We then conducted stratified sampling again to retrieve an even smaller dataset (sample size is 7,280) to use. Although we tried our best to keep information from the original dataset complete and sufficient, bias issues cannot be avoided or even worse, are not detected. Likewise, non-sampling errors will lead to sampling bias as well. Therefore, we may need to generate more heatmaps for different variables, and compare these results to official heat maps generated from much larger datasets (i.e. datasets from census.gov can be more reliable). 

# Chapter 4: Model Building
## Variable Selection$-$Feature Selection 
Here, we are dissecting which variables we should use in our model based on Adjusted R-Squared, Cp, BIC, and AIC. We are automatically excluding Census Tract and State variable because they have too many levels for this analysis. Additionally, we realize that we should be using bestglm() function to determine what variables to use for a logistic regression, but to have a comprehensive analysis we are going to try regsubsets() and bestglm() functions for model building. We know that our response variable is variable ‘Desert’ which is binary, but using forward, backwards, sequential, and exhaustive selection may provide us with some ideas about variable selection.
...
After running through all linear models the results showed us that the Region variable comes back as NA meaning this variable is useless to the model and does not seem to impact our dependent variable. We know that the difference between model_forward_adjr2 and model_forward_Cp is TractWhite, the summary of model_forward_adjr2 indicated that TractWhite should not be included in the model because it is not statistically significant, and anova() function's F test gave the same conclusion. When comparing model_forward_Cp to to model_forward_bic we can see that  model_forward_Cp is a better fit model based on the anova results. The model_forward_Cp fit uses all the variables except for TractBlack and TractWhite. We will keep this in mind and see how the backwards, sequential, and exhaustive methods align or contradict the results from forwards selection. 

### Backwards Selection
Now we will try backwards selection to see what variables are the best fit based on this method. Again, we realize that because our dependent variable is binary we will need to run a logistic regression. However, we want to have a comprehensive overview of the variables and their outcomes; this is why we are using feature selection to find the best fit linear model. 
...
Similar to forward selection, when we ran the different models the Region variable comes back as NA for the analysis fore each different region. The results from backwards selection based on the adjusted R-squared and Cp criterion show that the best fit model would not include the total Hispanic population in a census tract.  The BIC model also shows not to include the total Hispanic population. However, the best BIC model also says not to include median family income, the total Asian population, the total Black population, the total Native Hawaiian population,total White population, and the total multiple racial or other populations in a census tract. So because adjusted R-squared and Cp are the same we are only comparing the adjusted R-squared and BIC models. Based on the results, model_backward_adjr2 is a better choice.

### Sequential Selection 
After doing forwards and backwards selection to make the sequential selection easier, we removed variables 'TractBlack', 'TractWhite' and 'TractHispanic'. 
...
model_seq_adjr2<-lm(Desert~.-CensusTract-State-TractHispanic-TractBlack-TractSNAP, data= food)
...
The results from sequential selection for the adjusted R-squared and Cp criterion gave us the same variables. Meaning, we excluded the ones that we started with along with TractSNAP variable which denotes how many people are receiving SNAP benefits. So, we are only comparing adjusted R-squared with the BIC best fit model. Unlike the results from the forwards and backwards selection, the sequential selection gave us coefficients for the Region variable. However, for both the adjusted R-squared and BIC model the results are not statistically significant for this variable. When comparing these models we got a p-value smaller than 0.05 but larger than 0.1 from the output of F test. This tells us that by taking out the Asian population count variable and the White population count variable the BIC model is the better fit. Additionally, the summary of model_seq_adjr2 shows that 'TractAsian' should be excluded$-$which it is in the BIC model.  

### Exhaustive Selection

For the sequential method we removed the 'TractBlack', 'TractHispanic', and 'TractWhite' variables before finding the best model. For exhaustive we will remove these same variables along with 'TractSNAP' and 'TractAsian' per the results of the sequential model. 

```{R Exhaustive Variable Selection}
reg.exhaustive <-regsubsets(Desert~.-CensusTract-State-TractHispanic-TractBlack-TractSNAP-TractAsian, data = food, nvmax=19, nbest=1, method="exhaustive")
...
We tried exhaustive method here as an additional part, this method is only available when number of possible predictor variables is less than 20. Since we have removed all the variables that we have seen from the other methods we certainly have less than 20 variables to find the best fit model. However, after we take these variables out and look at the adjusted R-squared, Cp, and BIC results, the best fit models show all the other variables. Therefore, we do not need to run a regression with these variables because they are the same for each criteria. 

Again, we know that our dependent variable is Desert which is binary, and logistic regression should be used, but linear models still deserved to give us some insight about variable selection.

We did not include the variable 'State' in our model because there are too many levels, which increased the difficulty for us to draw conclusions, so we may drop it in the process of plotting. Yet, with no doubts, variable 'State' should be kept in the linear model try lm() part because it consists of so many levels so many levels. Some of these are significant, and this has been proven by the summary() function in the next step.

The biggest take-away from the the feature selection is that the Region variable is pretty useless in any analysis and we should not consider it in a logistic regression. However, we will look at what the bestglm () function tells us in the next chapter. 

## Chapter 4.1 
### Best GLM function for logistic regression - Variable Selection 
Now that we have looked at variable selection through feature selection and the 'leaps' package, we are now going to look into the bestglm method to find the best fit models for logistic regression. It is key to note that the glmbest () function does have limits. There is a 15 variable maximum, and categorical variables designated as factors are only designated to have two levels at most. Therefore, when deciding which variables to use, we are automatically excluding 'CensusTract', 'State', and 'Region'. Additionally, we will decide the rest of the variable selection based off the multicollinearity piece of our data. By controlling for multicollinearity will we be taking out the variables that are highly correlated to one another based off the correlation matrix. The first model that we are building will have the population variable in it. As such, we will exclude all the variables that were highly correlated with population - such as occupied housing units, total white population, and total kid population. Additionally, our correlation matrix told us that the 'TractSNAP' variable was highly correlated with the 'TractLOWI' variable. Due to the sequential selection suggesting the removal of 'TRACTSNAP', we will take out this variable in replace of the total amount of low-income people in a census tract. Also, the 'TractHispanic' variable was highly correlated with 'TractOMultir' or the total population of multi-racial individuals. However, because this variable is not well-defined we will exclude 'TractHispanic.' Then, for our second model we will be using the variables we excluded in our first model and exclude the variables that were included to maintain control over multicollinearity and test for the best fit model. 
...
The results above give us the top five models based on the criteria we selected. The following portion will assess each models further along with the models chosen from forwards selection to determine the best fit model from the AIC level. 

# Chapter 4.2: Model Selection 
## Step-Wise Selection with Dependent Variable as a Factor 
To continue with model selection we wanted to test whether or not the logistic regression made a difference when we classified the Desert variable as a factor with two levels. In this scenario, we are running the regressions to further determine the best fit model through step-wise by adding variables onto an empty general model. The empty general model is the intercept-only model. We start with this model to test forward selection, to better assess what variables to use for the best fit model. We realize that regardless if the variable 'Desert' is numerical or categorical, we get the same results, but for stringency, we will use variable 'Desertfactor' in the dataset named as 'newfood'.
```{r}
newfood<-food
...
### both<-step(emptyglm,scope=list(lower=formula(emptyglm),upper=formula(fullglm)),direction = "both")
### summary(both)
```

The results of the step-wise function for the forwards and backwards directions showed us different models. However, the forwards direction showed that the variable 'TractNHOPI' is not statistically significant with a p-value of 0.136183. Thus, we will use this model but take out this variable and this will match the results from the backwards direction. Furthermore, the results from classifying the direction as 'both' ways still ended up with the same results as the forwards direction. Moving forward in model building, we will we only be using the forwards model without TractNHOPI variable and we will call this forwards_2.

# Chapter 4.3: Model Selection 
## AIC Table - Best Fit Model 
Here we are using AIC levels to determine the best fit model to use for our logistical regression. We have multiple models from the glmbest () and forwards, backwards, sequential, and the step-wise model. Though when doing the glmbest() function our results did not let 'State' be apart of the model, we will be adding this variable to both of these models to test AIC. 

```{r results='markup'}
# Model Building Logistic Regression Based on Featured Selection Variables
...
model.seq <- glm(Desert~.-CensusTract-State-TractHispanic-TractBlack-TractSNAP-TractAsian, data=food, family="binomial")
# Taking out variables that were highly correlated with other variables 
emptyglm<-glm(Desert~1,data=food,family = binomial)
# Model Building from bestglm () function 
bestglm.1 <- glm(Desert~Urban+POP2010+NUMGQTRS+PovertyRate+TractLOWI+TractAsian+TractAIAN+TractHispanic+TractHUNV, data=food, family="binomial")
bestglm.1_state <-glm(Desert~Urban+POP2010+NUMGQTRS+PovertyRate+TractLOWI+TractAsian+TractAIAN+TractHispanic+TractHUNV+State, data=food, family="binomial")
bestglm.2 <- glm(Desert~Urban+OHU2010+NUMGQTRS+PovertyRate+MedianFamilyIncome+TractAsian+TractAIAN+TractOMultir+TractHUNV+TractSNAP, data=food, family="binomial")
bestglm.2_state <- glm(Desert~State+Urban+OHU2010+NUMGQTRS+PovertyRate+MedianFamilyIncome+TractAsian+TractAIAN+TractOMultir+TractHUNV+TractSNAP, data=food, family="binomial")
# Using Step-Wise Forward Selection Model  
forwards_2<-glm(Desert ~ MedianFamilyIncome + State + Urban + TractLOWI + 
    ...
    PovertyRate + TractNHOPI, data=food,family = binomial)
...
# Model AIC Levels 
models <- list(model.f, model.b, model.seq, emptyglm, bestglm.1, bestglm.1_state, bestglm.2, bestglm.2_state, forwards_2, forwards2_corr, model.lit)
model.names <- c("model.f", "model.b", "model.seq", "emptyglm", "bestglm.1", "bestglm.1_state", "bestglm.2", "bestglm.2_state", "forwards_2", "forwards2_corr", "model.lit")
aictab(cand.set=models, modnames=model.names)
```
As you can see from the table, out of all of our models the forwards_2 model that we got from the step-wise function is the best fit model. The AIC table shows us that by far this model has the smallest AIC, and because we are doing a logistic regression we want to use AIC as our final criteria for model selection. The AIC measures the maximum likelihood, which is why we want this value to be low. Additionally the AIC is the better criteria for explanatory models and we are wanting to assess what variables impact whether or not a census tract is in a food desert. Because we know that the forwards_2 model did not control for multicollinearity we wanted to use the variables from the step-wise result to asses for multicollinearity. That model is called forwards2_corr and as you can see taking out the variables that were highly correlated only increased the AIC level.

Additionally, we wanted to test how our bestglm models compared to the feature selection models and added them to the table. Unfortunately, the bestglm models are not considered good models based on their AIC level. This is interesting because the bestglm() function is supposed to help you pick the best variables for a logistic regression. We used AIC model selection to distinguish between a set of possible models describing potential indicators of food deserts. The best fit model, carrying 88% of the cumulative model weight was the forwards_2 model that included 15 variables to help explain the causes of food deserts.  
# Chapter 4.4: Logistic Regression 
Here we are running the logistic regression on the best fit model 'forwards_2'.
```{r results='markup'}
forwards_2<-glm(Desert ~ MedianFamilyIncome + State + Urban + TractLOWI + 
    TractHUNV + TractHispanic + TractAsian + NUMGQTRS + TractAIAN + 
    TractKids + TractSNAP + TractSeniors + POP2010 + OHU2010 + 
    PovertyRate,data=food,family = binomial)
summary(forwards_2)
exp(forwards_2$coefficients)
```
The above output shows you the coefficients of the forwards_2 model and what variables are statistically significant from this model. What is interesting to note is that states like Alaska and Florida are both statistically significant. At the beginning of the report we looked at a heat map of the food desert proportions across the country and both of these states had higher proportions compared to the rest of the nation. Additionally, we see that states like Michigan that are experiencing a mass wave of gentrification are also statistically significant. The variable Urban is significant with at the .001 level, meaning we are 99.9% confident that this variable has a positive impact on the likelihood of a census tract being in a food desert. The results of this table align with what we have seen in the literature around food deserts and the indicators of food deserts across the country. Furthermore, when you examine the exponentiated coefficients of the variables we can see that all the variables are positive. At the beginning of the report when we conducted normality test we were able to note that the distribution of our whole data set was positive and right-skewed and this is reiterated here.For example, the Urban coefficient is 4.36; so, the odd-ratio of census tract being located in a food desert increases by a factor of 4.36 when located in an urban area. We are 99.9% confident that the urban indicator has a positive relationship with a census tract being located in a food desert.

The begining of our analysis showed some interesing data around racial disparities and the means of racial populations in food deserts versus not in food deserts. Specifically, American Indian and Native Alaskan population were disproportionatally located in food desert tracts compared to not when we conducted our two sample t-test. Now, looking at our regression results this variable is statistically significant at the .001 level. This means that we are 99.9% confident that the odd-ratio of a census tract being a food desert increases when you are adding at least one additonal American Indian or Alaskan Native person to the census tract. The results of our regression are not suprising based on the exploratory data analysis that we completed earlier in this report.  

Below you can see the ROC curve of the forwards_2 logisitic regression. We can see that though our regression did not control for multicollinearity there is still some predicitive power from this model. 
```{r results='hide'}
loadPkg("pROC")
loadPkg("caret")
...
# Chapter 4.5 Optimization 
In the beginning of Chapter 1.1, we decided to sample our original dataset for efficiency, this helped our coding and interpretation process, meanwhile, it brought some classical sampling problems, such as sampling bias and model inaccuracy, we have mentioned in heatmap and model building part. To avoid such bias, the most direct solution is to increase our sample size. According to the Census population on April 1, 2010, the total population is around 328,000,000 (Fifty States plus D.C.), for normal computing power, it will be reasonable to ask for a sample size no more than 3,300,000, and the amount of observations collected in each state should follow  the population proportion. Then we can ensure that our data is complete and information is enough.
 
For columns, our dataset has many variables of the same kind, for example, variable ‘POP2010’, ‘TractKids’ and ‘TractSeniors’, usually, an area(county) with larger population, will certainly have more kids and seniors, the numbers of various races will also increase. Thus, they are highly correlated,  We always prefer more representative variables, for example, we can build a score variable to compute the quantity to describe numbers of different races, they can help solve vif issues and build a better model to get a more reliable prediction.

And we removed variables about exact addresses to simplify our EDA and model building, however, if we have a good equilibrium between computing power and model efficiency, we should include address variables, since they provide more detailed information about food desert situation in state level. For example, we know that it is more possible for inland areas to become food deserts because of external factors, such as transportation conditions and consumption level, but even two adjacent inland counties may have different situations, some food supply companies may choose to supply only one county with a higher consumption level to save expenditure. So if we have variables about exact locations, we can plot heatmaps with more details and explain further.

Also, we are willing to see the variable ‘desert’ become numeric (score) rather than binary. This will need some essential variables(supermarket density, consumption level, transportation, etc.) collected first and build a model to score every combination.
 
If the method above is applicable, the most obvious difficulty will be that larger data size means that we need more resources(funds, staff, etc...) to support the collection of data.


Potential Optimization Model:
VIF is a common issue for models with many variables, and we can ignore it if it is not serious(rule of thumb). If many VIFs larger than 10 are observed, ridge regression is suggested. We can remove some variables that have multicollinearity, this can be achieved by stepwise selection. PCA will be helpful as well.

# Chapter 5: Conclusion
To conclude, our analysis strongly aligned with the literature around food deserts. To answer our SMART question of what variables impact whether or not a census tract is in a food desert, we have proven that racial makeup of a neighborhood, geographic location, and socieconomic status of residents all have a positive relationship in indicating whether or not a census tract is located in a food desert. Furthermore, we were able to reject our null hypothesis because we have sufficient evidence that there are multiple indicators of whether or not a food desert is located in a Census Tract. Our null suggests that the given variables are independent of each other. Clearly, in the analysis portion of our work, it was evident that we were able to reject this as there was significance between many of the variables within the logistic model. As it so happens, this is what we did predict given the background information we have researched on food deserts. And so, as a result, our alternate hypothesis made more sense; there is a relationship between the variables assessed. 

The data suggests that some confounding factors are to be accounted for during future research if necessary. To begin with, our original dataset (sample size roughly ~ 72,000) was retrieved from USDA- this can be considered as sampling. We then conducted stratified sampling again to retrieve an even smaller data set (sample size is 7,280) to use, which was more manageable in R. At this point, we have done sampling twice already to the original dataset of n = 72,000. And so, although we tried our best to keep information from the original dataset complete and sufficient, bias issues cannot be avoided or even worse in some cases, are not detected. 

Likewise, non-sampling errors will lead to sampling bias as well. It may be wise, then, to generate more heatmaps for different variables, and compare these results to published literature looking at heatmaps generated from much larger datasets found like those found on USDA 


# Chapter 6: Bibliography 

Block, Jason., & S.V. Subramanian. (2015). Moving Beyond ‘Food Deserts’: Reorienting United States Policies to Reduce Disparities in Diet Quality. *PLOS Medicine, 12(12):* 1-9.  

Dutko, P.,Ploeg, M.V., & Farrigan, T. (2012, August) Characteristics and Influential Factors of Food Desserts. *Report from the Economic Research Team at USDA.* https://www.ers.usda.gov/webdocs/publications/45014/30940_err140.pdf

Hamidi, Shima. (2019). Urban sprawl and the emergence of food deserts in the USA. *Urban Studies* :1-16.

USDA. (2017) Food Access Research Atlas. https://www.ers.usda.gov/data-products/food-access-research-atlas/download-the-data/

Walker, R., Keane, C., & Burke, J. (2010). Disparities and access to healthy food in the United States: A review of food deserts literature. *Health & Place. 16(5):* 878-884. 
