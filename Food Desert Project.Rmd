...
>>>>>>> 331d65453353368f49a21e8cb697788c2cd1ef64
...
We chose to use the State variable because its representation is the most significant and it has obvious levels. The first step conducted was deciding to draw a dataset with a size of 7,280 rows to reflect the original dataset of 72,864 rows. Then we used the *table()* function to see the proportion of the State variable. Unfortunately, our results were not very satisfying. To simplify this process we used the calculations as seen below: ni representing the number of observations for State level i; wherein ceil stands for ceiling functions

$$a_i = ceil\left(\frac{n_i}{100}\right) x 10$$
$$N = \sum_{i=1}^{51}n_i = 71,864$$
$$A = \sum_{i=1}^{51}a_i = 7,280$$ 
         
We used the ceiling function because ai are not integers; we take ceiling values for ai to protect the integrity of levels by accommodating with having few rows. After this, “Sampling” package was installed  and loaded, strata() function was used to get stratified samples named as food. (See below for a readout of the dataset’s structure and variable names). However, once we extracted the sample food from the original dataset, we realized that we had 47 observations in which all of our variables were zero. To mend this issue in our analysis, we subsetted the *food* data frame to only include variables greater than zero (0>). As expected, this further decreased the data frame to 7,232 variables. For further analysis, we chose Desert as our single dependent variable. 
...
hist(food$TractHUNV, main="Histogram of Housing Units w/o a Vehicle", xlab="Total Hosuing Units w/o a Vehicle", col="orange")
...
Here, we are dissecting which variables we should use in our model based on Adjusted R-Squared, Cp, BIC, and AIC. We are automatically excluding Census Tract and State variable because they have too many levels for this analysis. Additionally, we realize that we should be using bestglm() function to determine what variables to use for a logistic regression, but to have a comprehensive analysis we are going to try regsubsets() and bestglm() functions for model building. We know that our response variable is variable ‘Desert’ which is binary, but using forward, backwards, sequential, and exhuastive selection may provide us with some ideas about variable selection.
...
After running through all linear models the results showed us that the Region variable comes back as NA meaning this variable is useless to the model and does not seem to impact our dependent variable. We know that the difference between model_forward_adjr2 and model_forward_Cp is TractWhite, the summary of model_forward_adjr2 indicated that TractWhiteshould not be included in the model because it is not statistically significant, and anova() function's F test gave the same conclusion. When comparing model_forward_Cp to to model_forward_bic we can see that  model_forward_Cp is a better fit model based on the anova results. The model_forward_Cp fit uses all the variables except for TractBlack and TractWhite. We will keep this in mind and see how the backwards, sequential, and exhaustive methods align or contradict the results from forwards selection. 

### Backwards Selection
Now we will try backwards selection to see what variables are the best fit based on this method. Again, we realize that becuase our dependent variable is binary we will need to run a logisitc regression. However, we want to have a comprehensive overview of the variables and their outcomes; this is why we are using feature selection to find the best fit linear model. 
...
## Chapter 4.1 
### Best GLM function for logisitc regression - Variable Selection 
...
Additionally, we wanted to test how our bestglm models compared to the feature selection models and added them to the table. Unfortunately, the bestglm models are not considered good models based on their AIC level. This is interesting because the bestglm() function is supposed to help you pick the best variables for a logisitc regression. We used AIC model selection to distinguish between a set of possible models describing potential indicators of food deserts. The best fit model, carrying 88% of the cumuluative model weight was the forwards_2 model that included 15 variables to help explain the causes of food deserts.  
