...
```{r Meian Asian Population Normality Test}
...
```{r American Indian and Alaskan Native Population }
...
Similarly,  we can conclude that variable 'State' and 'Desert' are not independent.

We want to check if all X-variables have a relationship with y-variable. Let's make this process quicker

```{R Chi-Square test between Y and X}
...
We chose to use 0.05 as the significance level, thus, for now, we can say that variable 'POP2010', 'OHU2010','MedianFamilyIncome','TractKids','TractSeniors','TractWhite','TractAsian','TractNHOPI' might not be good to use because there are no relationships between them and dependent variable 'Desert' based on the outputs of Chi-Square tests. But correlation matrix may give different results. It is better to make a final decision while building model.

After detecting relationship between Y and X, we need to see if all X are independent of each other, if we choose to do this one by one, it will be very time consuming (approximately 210 combinations) and hard to interpret, so we only pick out the variables that are possible to have relationship with others in common sense, for example, 'POP2010' and 'Urban' might not be independent.

```{R Chi-Square test between X's}
